		...
		SUGAR.util.extend(SUGAR.forms.VisibilityAction, SUGAR.forms.AbstractAction, {
			...
			exec: function(context)
			{
                ...
                try {
                    ...
                    //if there is a form name in the context, grab the field for the specific form
                    if(context.formName && context.formName.length > 0)
                    {
                       var formTarget = document.getElementById(context.formName);
                       if(formTarget)
                       {
                           targetName = this.target;
                           target = formTarget.elements[targetName];
                       }else{
                        //form is not in focus, there is no target to process, lets set
                        //target to null and not risk processing field with same name but not on this form
                            target = null;
                       }
                    }
