...
<input type="text" name="{{$idname}}" class={{if empty($displayParams.class) }}"sqsEnabled"{{else}} "{{$displayParams.class}}" {{/if}} tabindex="{{$tabindex}}" id="{{$idname}}" size="{{$displayParams.size}}" value="{{sugarvar key='value'}}" title='{{$vardef.help}}' autocomplete="off" {{$displayParams.readOnly}} {{$displayParams.field}}	{{if !empty($displayParams.accesskey)}} accesskey='{{$displayParams.accesskey}}' {{/if}} 
       {php} if($_REQUEST['return_module'] == 'Accounts') echo 'readonly="readonly" disabled="disabled"';{/php}>
...
onclick='open_popup(
	...
	true, 
	false, 
	{ldelim}"call_back_function":"set_return","form_name":"form_SubpanelQuickCreate_TDR_Type","field_to_name_array":{ldelim}"id":"tdr_type_accountsaccounts_ida","name":"tdr_type_accounts_name"{rdelim}{rdelim}, 
	"single", 
	true
);' {{if isset($displayParams.javascript.btn)}}{{$displayParams.javascript.btn}}{{/if}} {php} if($_REQUEST['return_module'] == 'Accounts') echo 'readonly="readonly" disabled="disabled"';{/php}><img src="{sugar_getimagepath file="id-ff-select.png"}"></button>{{if empty($displayParams.selectOnly) }}<button type="button" name="btn_clr_{{$idname}}" id="btn_clr_{{$idname}}" tabindex="{{$tabindex}}" title="{sugar_translate label="{{$displayParams.accessKeyClearTitle}}"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{{$idname}}', '{{if !empty($displayParams.idName)}}{{$displayParams.idName}}_{{/if}}{{sugarvar key='id_name'}}');"  value="{sugar_translate label="{{$displayParams.accessKeyClearLabel}}"}" {{if isset($displayParams.javascript.btn_clear)}}{{$displayParams.javascript.btn_clear}}{{/if}} {php} if($_REQUEST['return_module'] == 'Accounts') echo 'readonly="readonly" disabled="disabled"';{/php}><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
{{/if}}
</span>
{{/if}}
