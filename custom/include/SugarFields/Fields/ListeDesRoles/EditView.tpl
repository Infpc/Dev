...
{/if}
{* Modification du stagiaire le 9 Juillet 2014 *}
{literal}
	<script>
	function doStandardValidation() {
		// Original validation script
		var _form = document.getElementById('form_SubpanelQuickCreate_TDR_Type'); 
		disableOnUnloadEditView(); 
		_form.action.value='Save';
		if(check_form('form_SubpanelQuickCreate_TDR_Type')) return SUGAR.subpanelUtils.inlineSave(_form.id, 'TDR_Type_subpanel_save_button');
		return false;			
	}
	...
	$( document ).ready(function() {
		...
		var select = document.getElementById("role_sur_compte");
		if ($_GET['module'] == 'Accounts'){
		alert('');
			var account_id = $_GET['record'];
			var xhr = new XMLHttpRequest();
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					compteur = xhr.responseText;
					if (compteur >= 1) {
						if (select.options[select.selectedIndex].value == "GestionnaireOF") {
							select.disabled = true;
						}
						else {
							for (i=0;i<select.length;  i++) {
								if (select.options[i].value=='GestionnaireOF') {
									select.remove(i);
								}
							}
						}
			...
			xhr.open("GET","http://localhost/Reporting/count_of_managers.php?account_id=" + account_id,true);
			xhr.send();		
		}
		else if ($_GET['module'] == 'Contacts') {
			// Clear click events on buttons
			$('#form_SubpanelQuickCreate_TDR_Type div.action_buttons:first-child #TDR_Type_subpanel_save_button').removeAttr("onclick");
			$('#form_SubpanelQuickCreate_TDR_Type div.action_buttons:last-child #TDR_Type_subpanel_save_button').removeAttr("onclick");
			var select = document.getElementById("role_sur_compte");

			$('#form_SubpanelQuickCreate_TDR_Type div.action_buttons:first-child #TDR_Type_subpanel_save_button').click(function(event) {
				event.preventDefault();
				event.stopImmediatePropagation();
				var account_id = $("#tdr_type_accountsaccounts_ida").val();
				var xhr = new XMLHttpRequest();
				xhr.onreadystatechange = function() {
					if (xhr.readyState == 4) {
						compteur = xhr.responseText;
						if (compteur >= 1 && select.options[select.selectedIndex].value == "GestionnaireOF") {
							alert('Vous ne pouvez pas assigner plus d\'un gestionnaire OF \xE0 un compte\n( ' + $("#tdr_type_accounts_name").val() + ' poss\xE8de d\xE9j\xE0 un gestionnaire OF )');
						}
						else {
							doStandardValidation();
						}						
					}
				}
				xhr.open("GET","http://localhost/Reporting/count_of_managers.php?account_id=" + account_id,true);
			...
			$('#form_SubpanelQuickCreate_TDR_Type div.action_buttons:last-child #TDR_Type_subpanel_save_button').click(function(event) {
				event.preventDefault();
				event.stopImmediatePropagation();
				var account_id = $("#tdr_type_accountsaccounts_ida").val();
				var xhr = new XMLHttpRequest();
